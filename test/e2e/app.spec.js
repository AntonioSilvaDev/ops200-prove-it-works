...
describe('End to End Tests', () => {
  ...
  it('should contain a <input> element with name: "principal"', () => {
      return pageObject
        .evaluate(() => document.querySelector('input[name=principal]'))
        .then(input => expect(input).to.exist)
        });

  it('should contain a <input> element with name: "interestRate"', () => {
      return pageObject
        .evaluate(() => document.querySelector('input[name=interestRate]'))
        .then(input => expect(input).to.exist)
        });

  it('should contain a <input> element with name: "loanTerm"', () => {
      return pageObject
        .evaluate(() => document.querySelector('input[name=loanTerm]'))
        .then(input => expect(input).to.exist)
        });

  it('should contain a <select> element with name: "period"', () => {
      return pageObject
